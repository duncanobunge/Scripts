...
latest_vis_date as lastvisit,latest_tca as TCA,Current_Weight, art_dispensing_duration,tx_rtt,previousexpected_Date,rttdate,Screened_TB,
-- Screened_TB1,
ipt_start_date as ipt_initiation_date,
...
join 
(select patient_id, SUBSTRING_INDEX(GROUP_CONCAT(weight ORDER BY visit_date SEPARATOR '|'),'|',-1) AS Current_Weight from kenyaemr_datatools.triage
group by patient_id) dt on dt.patient_id=fup.patient_id
...
where date(visit_date) <= date_sub(   '2022-06-30', INTERVAL 1 DAY)  and program_name='HIV' and discontinuation_reason !='Lost to Follow'
group by patient_id -- check if this line is necessary
) d on d.patient_id = fup.patient_id
where fup.visit_date <= date_sub(   '2022-06-30', INTERVAL 1 DAY)
group by patient_id
--  we may need to filter lost to follow-up using this
having (
(((date(latest_tca) < date_sub(   '2022-06-30', INTERVAL 1 DAY)) and (date(latest_vis_date) < date(latest_tca))) ) and ((date(latest_tca) > date(date_discontinued) and date(latest_vis_date) > date(date_discontinued)) or disc_patient is null ) and datediff(date_sub(   '2022-06-30', INTERVAL 1 DAY), date(latest_tca)) > 30)
...
from kenyaemr_datatools.hiv_followup
where next_appointment_date < date_sub(   '2022-06-30', INTERVAL 30 DAY)
group by patient_id
...
where next_appointment_date >    '2022-06-30' group by patient_id
...
-- left join(SELECT patient_id,visit_date,next_appointment_date as tca FROM kenyaemr_datatools.hei_follow_up_visit where next_appointment_date>   '2022-06-30'
...
select patient_id,
visit_date,
-- SUBSTRING_INDEX(GROUP_CONCAT(screened_for_tb ORDER BY visit_date SEPARATOR '|'),'|',-1) AS Screened_TB1,
mid(max(concat(visit_date,screened_for_tb)),11) as Screened_TB
from kenyaemr_datatools.hiv_followup -- where visit_date between '2022-05-31' and '2022-06-30'
group by patient_id
) tbs on tbs.patient_id=fup.patient_id
...
UPN as ccc_number,dob as date_of_birth,gender,entry_point as Patient_Source,transferin_date,enroll_date as date_enrolled,art_start_date as Date_initiated_art,who_stage_enrollment,current_regimen,regimen_line as current_regimen_line,current_regimen_date,
latest_vis_date as lastvisit,latest_tca as TCA,Current_Weight,
art_dispensing_duration,tx_rtt,previousexpected_Date,rttdate,Screened_TB,-- Screened_TB1,
ipt_start_date as ipt_initiation_date,
ipt_outcome_date,ipt_outcome as ipt_status,presumed_tb as presumptive_tb,
date_assessed as date_assessed_stability,
